...
import com.example.glidedemo.views.pinlockview.ResourceUtils;
...
public class PatternLockView extends View {
    ...
    private DotState[][] mDotOutStates;
    ...
    private int mNormalOutStateColor;
    private int mNormalInnerStateColor;
    private int mWrongStateColor;
    private int mCorrectStateColor;
    private int mPathWidth;
    private int mDotNormalSize;
    private int mDotOutNormalSize;
    /**
     * 自动清除
     */
    private boolean mEnableAutoClean;
    /**
    ...
    public PatternLockView(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mAspectRatioEnabled = typedArray.getBoolean(R.styleable.PatternLockView_aspectRatioEnabled,
                    false);
            mEnableAutoClean = typedArray.getBoolean(R.styleable.PatternLockView_enableAutoClean,
                    false);
            mFreezeDuration = typedArray.getInt(R.styleable.PatternLockView_freezeDuration,
                    500);
            ...
            mPathWidth = (int) typedArray.getDimension(R.styleable.PatternLockView_pathWidth,
                    ResourceUtils.getDimensionInPx(getContext(), R.dimen.dp_3));
            ...
            mNormalOutStateColor = typedArray.getColor(R.styleable.PatternLockView_normalOutStateColor,
                    ResourceUtils.getColor(getContext(), R.color.lock_out));
            mNormalInnerStateColor = typedArray.getColor(R.styleable.PatternLockView_normalInnerStateColor,
                    ResourceUtils.getColor(getContext(), R.color.lock_inner));
            //完成的线
            mCorrectStateColor = typedArray.getColor(R.styleable.PatternLockView_correctStateColor,
                    ResourceUtils.getColor(getContext(), R.color.white));
            mWrongStateColor = typedArray.getColor(R.styleable.PatternLockView_wrongStateColor, Color.RED);
            mDotNormalSize = (int) typedArray.getDimension(R.styleable.PatternLockView_dotNormalSize,
                    ResourceUtils.getDimensionInPx(getContext(), R.dimen.dp_10));
            mDotOutNormalSize = (int) typedArray.getDimension(R.styleable.PatternLockView_dotOutNormalSize,
                    ResourceUtils.getDimensionInPx(getContext(), R.dimen.dp_26));
            mDotSelectedSize = (int) typedArray.getDimension(R.styleable
                            .PatternLockView_dotSelectedSize,
                    ResourceUtils.getDimensionInPx(getContext(), R.dimen.dp_24));
        ...
        for (int i = 0; i < sDotCount; i++) {
            for (int j = 0; j < sDotCount; j++) {
                sDots[i][j] = new Dot(sDotCount, i, j);
            }
        }

        mDotStates = new DotState[sDotCount][sDotCount];
        mDotOutStates = new DotState[sDotCount][sDotCount];
        for (int i = 0; i < sDotCount; i++) {
            for (int j = 0; j < sDotCount; j++) {
                mDotStates[i][j] = new DotState();
                mDotStates[i][j].mSize = mDotNormalSize;
                mDotOutStates[i][j] = new DotState();
                mDotOutStates[i][j].mSize = mDotOutNormalSize;
            }
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < sDotCount; i++) {
            ...
            for (int j = 0; j < sDotCount; j++) {
                ...
                DotState dotOutState = mDotOutStates[i][j];
                float outCenterX = getCenterXForColumn(j);
                float outSize = dotOutState.mSize * dotOutState.mScale;
                float outTranslationY = dotOutState.mTranslateY;
                drawOutCircle(canvas, (int) outCenterX, (int) centerY + outTranslationY,
                        outSize, drawLookupTable[i][j], dotOutState.mAlpha);
        ...
        if (drawPath) {
            mPathPaint.setColor(getCurrentColor(false));
    ...
    public int getDotOutNormalSize() {
        return mDotOutNormalSize;
    }
    ...
    public void setNormalOutStateColor(@ColorInt int normalOutStateColor) {
        mNormalOutStateColor = normalOutStateColor;
    }

    public void setNormalInnerStateColor(@ColorInt int normalInnerStateColor) {
        mNormalInnerStateColor = normalInnerStateColor;
    }
    ...
    public void setDotOutNormalSize(@Dimension int dotOutNormalSize) {
        mDotOutNormalSize = dotOutNormalSize;
        for (int i = 0; i < sDotCount; i++) {
            for (int j = 0; j < sDotCount; j++) {
                mDotOutStates[i][j] = new DotState();
                mDotOutStates[i][j].mSize = mDotOutNormalSize;
    ...
    public void setEnableAutoClean(boolean enableAutoClean) {
        mEnableAutoClean = enableAutoClean;
    }
    ...
    private void startTimer() {
        ...
        this.postDelayed(this.action, mFreezeDuration);
    }
    ...
    private void drawCircle(Canvas canvas, float centerX, float centerY,
        ...
        mDotPaint.setColor(getInnerColor(partOfPattern));
        mDotPaint.setAlpha((int) (alpha * 255));
        canvas.drawCircle(centerX, centerY, (float) (size * 0.75), mDotPaint);
    }


    private int getInnerColor(boolean partOfPattern) {
        if (partOfPattern) {
            return ResourceUtils.getColor(getContext(), R.color.white);
        } else {
            return ResourceUtils.getColor(getContext(), R.color.lock_inner);
        }

    }

    private void drawOutCircle(Canvas canvas, float centerX, float centerY,
                               float size, boolean partOfPattern, float alpha) {
        mDotPaint.setColor(mNormalOutStateColor);
//        mDotPaint.setAlpha((int) (alpha * 255));
        canvas.drawCircle(centerX, centerY, size / 2, mDotPaint);
    }

    public static synchronized Dot dotOf(int row, int column) {
        checkRange(row, column);
        return sDots[row][column];
    }

    public static synchronized Dot dotOf(int id) {
        return dotOf(id / sDotCount, id % sDotCount);
    }
