...
open class PatternLockerView @JvmOverloads constructor(
    ...
    private fun drawCells(canvas: Canvas) {
        this.cellBeanList.forEach {
            if (it.isHit && hitCellView != null) {
                this.hitCellView?.draw(canvas, it, isError)
            } else {
    ...
    private fun handleActionMove(event: MotionEvent) {
        ...
        val size = hitIndexList.size
        if (this.hitSize != size) {
            this.hitSize = size
            this.listener?.onChange(this, hitIndexList)
        }
    }
    ...
    private fun handleActionUp(event: MotionEvent) {
        ...
        //3. notify listener
        this.listener?.onComplete(this, hitIndexList)


        //4. startTimer if needed
        if (this.enableAutoClean && hitIndexList.size > 0) {
            this.startTimer()
        }
    }
    ...
    private fun updateHitState(event: MotionEvent) {
        this.cellBeanList.forEach {
            if (!it.isHit && it.of(event.x, event.y)) {
                if (!enableSkip && hitIndexList.isNotEmpty()) {
                    val last = cellBeanList[this.hitIndexList.last()]
    ...
    private fun startTimer() {
        ...
        this.postDelayed(this.action, freezeDuration.toLong())
    }
